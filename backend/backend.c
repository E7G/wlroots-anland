...
struct wlr_allocator *backend_get_allocator(struct wlr_backend *backend) {
	...
	if (wlr_backend_is_anland(backend)) {
		...
		backend->allocator = wlr_anland_allocator_create(backend, renderer);
		return backend->allocator;
	}
...
static struct wlr_backend *attempt_backend_by_name(struct wl_display *display,
	...
	if (strcmp(name, "wayland") == 0) {
		...
		return wlr_anland_backend_create(display, getenv("ANLAND_SOCKET"));
...
struct wlr_backend *wlr_backend_autocreate(struct wl_display *display) {
	...
	/* Anland is opt-in: setting ANLAND_SOCKET is enough to run labwc without
	 * requiring a DRM master or a host compositor. WLR_BACKENDS=anland remains
	 * available for explicit multi-backend configurations. */
	if (getenv("ANLAND_SOCKET")) {
		struct wlr_backend *anland = wlr_anland_backend_create(display,
			getenv("ANLAND_SOCKET"));
		if (!anland || !wlr_multi_backend_add(backend, anland)) {
			if (anland) wlr_backend_destroy(anland);
			goto error;
		}
		return backend;
	}
