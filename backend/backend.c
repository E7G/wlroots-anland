...
static bool attempt_backend_by_name(struct wl_event_loop *loop,
	...
	if (strcmp(name, "wayland") == 0) {
		...
		backend = wlr_anland_backend_create(loop, getenv("ANLAND_SOCKET"));
...
struct wlr_backend *wlr_backend_autocreate(struct wl_event_loop *loop,
	...
	if (getenv("ANLAND_SOCKET")) {
		struct wlr_backend *anland = wlr_anland_backend_create(loop, getenv("ANLAND_SOCKET"));
		if (!anland) {
			goto error;
		}
